#!/usr/bin/env bash
#
# Apply Prisma migrations to a RavelGo RDS database (staging or production).
#
...
#
# Usage:  bash scripts/migrate-env.sh <staging|production>
# Prefer the per-environment wrappers instead: scripts/migrate-staging.sh /
...
ENVNAME="${1:?Usage: bash scripts/migrate-env.sh <staging|production>}"
case "${ENVNAME}" in
  staging|production) ;;
  *) echo "ERROR: unknown environment '${ENVNAME}' (expected 'staging' or 'production')"; exit 1 ;;
esac

# Matches the stack-naming convention in infra/bin/infra.ts: production
# stacks carry no suffix (RavelGo-Network, RavelGo-Data, ...), every other
# environment is suffixed (RavelGo-Network-staging, RavelGo-Data-staging, ...).
SUFFIX=""
...
if [ "${ENVNAME}" = "production" ]; then
  echo "==> Target: PRODUCTION. This applies pending migrations to the live database."
...
VPC_ID="$(aws ec2 describe-vpcs \
  --filters "Name=tag:aws:cloudformation:stack-name,Values=RavelGo-Network${SUFFIX}" \
...
DB_SECRET_ARN="$(aws cloudformation describe-stacks --stack-name "RavelGo-Data${SUFFIX}" \
...
if [ "${ST}" = "SUCCEEDED" ]; then
  echo "✅ Migrations applied to ${ENVNAME}. Your database now has its tables."
else
