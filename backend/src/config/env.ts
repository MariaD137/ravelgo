...
const rawEnvSchema = z.object({
  ...
  // The assets bucket's CloudFront domain (infra/lib/storage-stack.ts's
  // AssetsDistribution) — lets the backend build a public URL for something
  // uploaded there (e.g. a vehicle photo) without ever signing a GET. Optional
  // so dev/test boots without it; a stored photoKey with no domain configured
  // just means photoUrl comes back null rather than a broken link.
  ASSETS_CDN_DOMAIN: z.string().optional(),
