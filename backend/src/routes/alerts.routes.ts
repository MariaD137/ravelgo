...
alertsRouter.post("/emergency-alerts", requireAuth, asyncHandler(async (req, res) => {
...
alertsRouter.get("/emergency-alerts/mine", requireAuth, asyncHandler(async (req, res) => {
  const user = await prisma.user.findUnique({ where: { cognitoSub: req.user!.sub } });
  if (!user) return res.status(404).json({ error: "User profile not found" });
...
alertsRouter.get("/emergency-alerts", requireAuth, requireRole("Admin"), asyncHandler(async (req, res) => {
...
alertsRouter.patch("/emergency-alerts/:id/status", requireAuth, requireRole("Admin"), asyncHandler(async (req, res) => {
