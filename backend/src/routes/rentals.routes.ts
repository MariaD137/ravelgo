...
rentalsRouter.post("/rentals", requireAuth, requireRole("Driver"), asyncHandler(async (req, res) => {
...
rentalsRouter.get("/rentals/mine", requireAuth, requireRole("Driver"), asyncHandler(async (req, res) => {
  const driver = await findOwnDriver(req.user!.sub);
  if (!driver) return res.status(404).json({ error: "Driver profile not found" });
...
rentalsRouter.get("/rentals", requireAuth, asyncHandler(async (req, res) => {
...
rentalsRouter.patch("/rentals/:id/status", requireAuth, requireRole("Admin"), asyncHandler(async (req, res) => {
