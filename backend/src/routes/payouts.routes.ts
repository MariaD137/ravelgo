...
/** True if `err` is a Prisma unique-constraint violation — used to turn a
 * lost create-payout race into a clean 409 rather than a 500. The
 * check-then-create above each call site is a fast path for the common
 * case; the @@unique([driverId, period]) constraint is the real backstop,
 * since only the database sees every concurrent request. */
...
payoutsRouter.post("/payouts/create", requireAuth, requireRole("Admin"), async (req, res, next) => {
  try {
    ...
    // Fast path: catches the common case (no concurrent request) with a
    // friendly error before doing any real work. Not itself race-safe —
    // two requests can both pass this check before either commits — so the
    // @@unique([driverId, period]) constraint below is the actual
    // protection; the catch block turns that into the same friendly 409
    // instead of a 500 when the fast path loses the race.
    ...
    try {
      ...
      if (isDuplicatePayoutError(err)) {
        const raced = await withBypass((tx) => tx.payout.findFirst({ where: { driverId, period } }));
        return res.status(409).json({ error: `Payout already exists for ${period}`, payoutId: raced?.id });
      }
      throw err;
    }
