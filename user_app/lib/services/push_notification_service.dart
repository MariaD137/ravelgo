...
import 'package:ravelgo_user_app/services/push_token_api.dart';
...
/// authenticates a user; nothing here calls Amplify.Auth.signIn or reads
/// its session.
///
...
/// .env — exactly like ApiClient.isConfigured / Currency.symbol elsewhere in
/// this app degrade when their own .env values are absent.
class PushNotificationService {
  ...
  // try/catch matches Currency.symbol/ApiClient.isConfigured's own pattern:
  // dotenv.env throws NotInitializedError if dotenv.load() hasn't run yet
  // (e.g. a widget test that never calls it) — reading these must never
  // crash the app, it should just mean "not configured".
  ...
  /// Called when the user taps a delivered push notification. Reads the same
  /// referenceType/referenceId data payload the backend attaches to every
  /// push (see backend/src/lib/notifications.ts) — actual navigation is
  /// wired by whichever screen is listening via [onNotificationTapped].
