...
class AuthService {
  ...
  Future<Map<String, dynamic>> signUp(String email, String password, String name, String phone) async {
    ...
    final request = await http.postUrl(uri);
    request.headers.set('Content-Type', 'application/x-amz-json-1.1');
    request.headers.set('X-Amz-Target', 'AWSCognitoIdentityProviderService.SignUp');
    request.write(jsonEncode({
      'ClientId': _clientId,
      'Username': email,
      'Password': password,
    ...
    if (response.statusCode != 200) {
      final errorType = body['__type'] ?? 'UnknownError';
      final message = body['message'] ?? 'Sign up failed';
      return {'success': false, 'error': _friendlyError(errorType, message)};
    }

    return {'success': true, 'userConfirmed': body['UserConfirmed'] ?? false};
  }
  ...
  Future<Map<String, dynamic>> confirmSignUp(String email, String code) async {
    ...
    final request = await http.postUrl(uri);
    request.headers.set('Content-Type', 'application/x-amz-json-1.1');
    request.headers.set('X-Amz-Target', 'AWSCognitoIdentityProviderService.ConfirmSignUp');
    request.write(jsonEncode({
      'ClientId': _clientId,
      'Username': email,
  ...
  String _friendlyError(String type, String message) {
    switch (type) {
      ...
      case 'InvalidPasswordException':
        return 'Password does not meet requirements (min 8 chars, uppercase, lowercase, number)';
