...
void main() {
  test('get() attaches the bearer token and returns decoded JSON on 200', () async {
    ...
    final result = await api.get('/api/trips/mine');
  ...
  test('get() omits Authorization header when no token is available', () async {
    ...
    await api.get('/api/trips/mine');

    expect(captured.headers.containsKey('Authorization'), isFalse);
  });

  test('a 409 RENTAL_OVERLAP response throws ApiException carrying the backend conflict shape', () async {
    ...
    await expectLater(
      api.post('/api/rentals/rl-1/bookings', body: {}),
      throwsA(
        isA<ApiException>()
            .having((e) => e.statusCode, 'statusCode', 409)
            .having((e) => e.code, 'code', 'RENTAL_OVERLAP'),
      ),
    );
  });
