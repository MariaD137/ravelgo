...
/// Cognito sign-in flow yet (see PRE_AWS_STRIPE_READINESS.md) — but once
/// one is wired up (via amplify_auth_cognito or an equivalent native SDK,
/// against a real User Pool app client this change must not invent or
/// hardcode), a real implementation of this same interface drops in without
/// touching ApiClient, the domain API classes, or any screen — they all
/// already depend on this abstraction, not on Cognito.
...
/// security control: it stores nothing durably or securely, and
/// [login] deliberately fails loudly rather than fabricating a token that
...
/// in effect — every other constructor (ApiClient's default, DriverSession's
/// composition in main.dart, a screen's default parameter) calls this
/// instead of choosing for itself, so there is exactly one `kReleaseMode`
/// branch to audit. Swap the non-release branch for a real
/// `CognitoAuthProvider` once a User Pool exists; the release branch must
/// stay [UnauthenticatedAuthProvider] until a real one is ready to replace
/// it — never [DevOnlyAuthProvider], which would throw, and never anything
/// that fabricates a session.
