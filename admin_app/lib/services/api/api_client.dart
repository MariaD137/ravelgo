...
/// Thin, shared HTTP wrapper used by every domain API class (rider_api.dart,
/// driver_api.dart, document_api.dart, rental_api.dart, courier_api.dart,
/// pricing_api.dart, payout_api.dart, support_api.dart, dashboard_api.dart)
/// instead of each calling package:http directly. Resolves the backend base
/// URL from .env (API_BASE_URL — declared in .env.example), attaches the
/// caller's bearer token via [AuthProvider], and turns a non-2xx response, a
/// network failure, a timeout, or a malformed response into a single,
/// uniformly catchable [ApiException]. Auth, base URL resolution, and error
/// handling all live in exactly this one place — mirrors driver_app's and
...
class ApiClient {
  ...
  /// Resolves a possibly-relative asset URL (e.g. a rental listing's
  /// vehicle.imageUrl like "/demo-assets/demo-bmw-7-series.png" from a
  /// seeded demo vehicle) against API_BASE_URL, the same way every REST
  /// path already is. A real driver-uploaded photo comes back as an
  /// absolute CloudFront URL (see backend/src/services/assets.ts) and is
  /// returned unchanged. Null/empty input returns null, so callers can pass
  /// a vehicle's imageUrl straight through without a null check first.
